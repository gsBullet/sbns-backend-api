...
const removeImageFile = (imageUrl) => {
  ...
  try {
    ...
    if (!filename) return;
    const filePath = path.join(__dirname, "..", "..", "uploads", "our-hero", filename);
...
module.exports = {
  getOurAllHeroes: async (req, res) => {
    try {
      ...
      return res.status(200).json({ success: true, data: response, message: "Martyred leaders fetched successfully" });
    } catch (error) {
      console.error(error);
  ...
  createOurHero: async (req, res) => {
    const { initial, name, role, bio, tags } = req.body;
    try {
      const newOurHero = await MartyredLeaderModel.create({
        initial, name, role, bio, tags,
        image: req.file ? buildImageUrl(req, req.file.filename) : "",
      });
      return res.status(201).json({ success: true, data: newOurHero, message: "Martyred leader created successfully" });
    } catch (error) {
      console.error(error);
  ...
  updateOurHeros: async (req, res) => {
    ...
    try {
      ...
      if (!existing) {
        return res.status(404).json({ success: false, message: "Martyred leader not found" });
      }

      const update = { initial, name, role, bio, tags };
      ...
      return res.status(200).json({ success: true, data: updatedOurHero, message: "Martyred leader updated successfully" });
    } catch (error) {
      console.error(error);
      return res.status(400).json({ success: false, message: error.message });
    }
  ...
  updateOurHerosStatus: async (req, res) => {
    try {
      ...
      return res.status(200).json({ success: true, data: updatedOurHero, message: "Martyred leader status updated successfully" });
    } catch (error) {
      console.error(error);
      return res.status(400).json({ success: false, message: error.message });
    }
  ...
  deleteOurHero: async (req, res) => {
    try {
      const deletedOurHero = await MartyredLeaderModel.findByIdAndDelete(req.params.id);
      if (!deletedOurHero) {
        return res.status(404).json({ success: false, message: "Martyred leader not found" });
      }
      removeImageFile(deletedOurHero.image);
      return res.status(200).json({ success: true, data: deletedOurHero, message: "Martyred leader deleted successfully" });
    } catch (error) {
      console.error(error);
      return res.status(400).json({ success: false, message: error.message });
    }
